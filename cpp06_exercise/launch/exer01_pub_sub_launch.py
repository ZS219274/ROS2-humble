...
def generate_launch_description():
    ...
    # 3.调用自定义的节点，并且该节点调用顺序有要求（要求掉头完毕才能执行）
    exer01 = Node(package="cpp06_exercise", executable="exer01_pub_sub")
    # 怎么控制节点的创建顺序呢（该节点需要在乌龟完成掉头动作后再创建），主要通过注册事件完成
    # 创建事件注册对象， 在对象中声明针对哪个节点，在哪个事件触发时，执行哪个操作
