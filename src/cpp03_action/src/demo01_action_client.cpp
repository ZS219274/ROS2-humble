...
public:
    explicit MinimalActionClient(const rclcpp::NodeOptions & node_options =
        rclcpp::NodeOptions()) : Node("minimal_action_client", node_options){
    // 3-1.创建动作客户端；
...
private:
    ...
    // 3-4.处理连续反馈；
    void feedback_callback(GoalHandleProgress::SharedPtr,const std::shared_ptr<const
