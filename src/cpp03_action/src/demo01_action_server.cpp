/*
*    需求：编写动作服务端实习，可以提取客户端请求提交的整型数据，并累加从1 到该数据之间的所有整数以求和，
...
public:
    explicit MinimalActionServer(const rclcpp::NodeOptions & options =
        rclcpp::NodeOptions()) : Node("minimal_action_server", options){
        // 3-1.创建动作服务端；
...
private:
    ...
    // 3-4.生成连续反馈。//耗时操作，建议作异步任务来执行（开线程，future）
